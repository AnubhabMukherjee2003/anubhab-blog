...
export default async function Page() {
  ...
  return (
    <div className="w-full min-h-screen p-[clamp(0.5rem,3vw,2rem)] ">
          ...
          <div className="grid gap-2 md:grid-cols-[minmax(80px,10vw)_1fr] md:gap-6">
            {blogPosts.map((post) => (
              <Fragment key={post.slug}>
                ...
                <div className="py-2 md:py-4 border-b border-transparent hover:border-[#444] transition md:col-span-1 group">
                  <Link
                    href={`/${post.slug}`}
                    className="block font-medium text-base md:text-lg text-white group-hover:text-[#999] transition-colors duration-200"
                  >
      ...
      <footer className="mt-8 py-4 text-center text-xs text-[#999]">
